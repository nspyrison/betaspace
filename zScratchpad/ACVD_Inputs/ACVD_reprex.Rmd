...
alpha.df <- phyloseq::estimate_richness(
  full_meta, measures = c("Shannon", "InvSimpson", "Observed", "Fisher")) %>%
  as_tibble(rownames = "SampleID") %>%
  left_join(phyloseq::sample_data(physeq), by = 'SampleID')

alpha.df.l <- alpha.df %>%
  pivot_longer(cols = c("Shannon", "InvSimpson", "Observed", "Fisher"),
               names_to = 'Diversity_Measure', values_to = 'value')

alpha.box1 <- ggplot(alpha.df.l) +
...
#' @examples
#' # Quest for distance matrix 
#' ## Try to get dist...
#' physeq.beta$call$distance
#' ## Null..., so recreate it...
#' bray_dist <- vegan::vegdist(phyloseq::otu_table(full_meta), method = "bray")
...
# This returns a numeric vector of the percentages (e.g., 12.01, 7.30)
pct_exp <- round(physeq.beta$values$Relative_eig[1:d] * 100, 1)
pca.df <- data.frame(physeq.beta$vectors)
colnames(pca.df) <- paste0("PC", 1:ncol(pca.df), " (", pct_exp, "%)")
...
pca.plot <- ggplot(pca.df) +
  geom_point(aes(x = `PC1 (12%)`, y = `PC2 (7.3%)`, color = ACVD), size = 2.5) +
  stat_ellipse(aes(x = `PC1 (12%)`, y = `PC2 (7.3%)`, color = ACVD),
               level = 0.99, linewidth = 1) +
  scale_color_manual(values = acvd_colors) +
  my_gg_theme +
  labs(subtitle = 'PCoA1:2, Bray-Curtis Distance', color = "Heart\nDisease")
pca.plot
```
...
```{r}
jie_alpha_wide <- alpha.df
jie_alpha_long <- alpha.df.l
jie_bray_pcoa <- physeq.beta
jie_bray_pcoa_embed <- pca.df
```

## 8.1 Make CAP

```{r}
# CAP with a simple constraint (e.g., ACVD status. Could also add any number of predictors)
jie_bray_cap <- phyloseq::ordinate(
  full_meta, method = 'CAP', distance = 'bray', formula = ~ ACVD)
  
# Different display options for CAP scores:
# Sites (sample coordinates) - the main embedding
jie_bray_cap_embed <- vegan::scores(
  jie_bray_cap, display = "sites", choices = 1:9999) %>% as.data.frame()
jie_bray_cap_embed <- tibble(
  jie_bray_cap_embed[, 1:4],
  SampleID = rownames(jie_bray_cap_embed))
jie_bray_cap_embed <- left_join(jie_bray_cap_embed, metadata, by = 'SampleID') %>%
  mutate(bmi_bin = cut(BMI, breaks = c(0, 18, 25, 30, 35, 40, 50, 70),
                       include.lowest = TRUE, right = FALSE, dig.lab = 4))
```
...
```{r}
save(jie_alpha_wide, jie_alpha_long,
     jie_bray_pcoa, jie_bray_pcoa_embed, jie_bray_cap_embed,
     file = paste0(proj_path, "outputs/ns_diversity_spaces.rda"))
```
