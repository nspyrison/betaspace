...
subtitle: 
author: "Madison Youngblom (madison.youngblom@iff.com)"
...
## Filenames
metadata_file <- './inputs/Jieetal_2017_supp.txt'
read_summary <- './inputs/Read_counts_quality.txt'
metaphlan_unclass <- './inputs/merged_metaphlan_count_species_wUnclass.txt'
...
# Set global options for all DT tables in the Rmd
options(DT.options = list(
  pageLength = 10, # Default number of rows to display
  autoWidth = FALSE, # Prevent automatic width adjustment (useful for custom widths)
  scrollX = TRUE # Enable horizontal scrolling if table is too wide
  # Add other common options here
  # columnDefs = list(list(className = 'dt-center', targets = "_all")) # Example to center all columns
))

# You can also set a global style, e.g., Bootstrap styling
# options(DT.style = "bootstrap") # Requires the style = 'bootstrap' argument in datatable() for full effect
```
...
### 2.3 Reads mapping to unclassified taxa
...
df_unclass <- read.delim (metaphlan_unclass, 
                             header = TRUE,
                             sep = "\t", 
                             check.names = FALSE,
                             skip = 1) %>%
...
g <- ggplot(alpha.df) +
  geom_point(aes(x=Observed,y=Shannon,
                 color=ACVD,),size=3) +
...
pca.df <- left_join(pca.df,metadata,by='SampleID') %>%
    mutate(bmi_bin = cut(BMI, breaks = c(0,18,25,30,35,40,50,70), include.lowest = TRUE, right = FALSE, dig.lab = 4))
...
pca.plot <- ggplot(pca.df) +
  ...
  stat_ellipse(aes(x=PCoA1,y=PCoA2, color=ACVD),level = 0.99, linewidth=1) +
...
physeq_nounclass_rel <- transform_sample_counts(physeq_nounclass, function(x) x/sum(x))

core <- aggregate_rare(physeq_nounclass_rel, level="Family", detection = 0.01, prevalence = 0.2)
...
prevalence.est<-function(df, taxa.are.rows = T){
  ...
  summ<-df %>% group_by(Species,ACVD) %>% summarize(count=sum(abundance),.groups = 'drop')
  return(summ)
}

df.prev <- prevalence.est(abd_akks)

prev <- metadata %>% group_by(ACVD) %>% summarize(sample_count=n(),.groups = 'drop') %>%
...
DT::datatable(summ %>% filter(Species %in% c('Akkermansia_muciniphila','Akkermansia_massiliensis','Akkermansia_guangxiensis')),
...
```{r diffabund, eval=FALSE, include=FALSE}
# references
# helpful post: https://forum.biobakery.org/t/pairwise-comparisons-for-interaction-terms-in-maaslin3/7886
# contrast tests: https://github.com/biobakery/biobakery/wiki/maaslin3#46-contrast-tests
# formula help: https://forum.biobakery.org/t/longitudinal-analysis-using-maaslin3/7900/4

# calculating using relative abundance values calculated from metaphlan counts using phyloseq (instead of rel abd values directly from metaphlan)
physeq_rel <- transform_sample_counts(physeq, function(x) x/sum(x))
physeq_smg_taxa <- as.data.frame(tax_table(physeq_rel)) %>%
  rownames_to_column(var = 'metaphlan_species') %>%
  select(metaphlan_species,Species)
...
metadata_wreads <- metadata %>%
  left_join(seq %>% select(SampleID=Sample,Reads="De-contaminated Reads (M)"), by = "SampleID") %>%
  mutate(bmi_category = factor(case_when(is.na(BMI) ~ NA,
                                  BMI < 25 ~ 'healthy',
                                  TRUE ~ 'overweight'),levels = c('healthy','overweight')))
rownames(metadata_wreads) <- metadata_wreads$SampleID

ggplot(metadata_wreads) +
  geom_boxplot(aes(x=ACVD,y=BMI,fill=ACVD)) +
  scale_fill_manual(values=acvd_colors) +
  my_gg_theme
ggplot(metadata_wreads) +
  geom_boxplot(aes(x=ACVD,y=Age,fill=ACVD)) +
  scale_fill_manual(values=acvd_colors) +
  my_gg_theme

# run different formulas
maaslin_species1 <- maaslin3(input_data = df_rel,
                           input_metadata = metadata_wreads,
                           output = './outputs/maaslin_results/acvd_age_reads',
                           formula = "~ ACVD + Age + Reads",
                           normalization = 'TSS',
                           transform = 'LOG',
                           cores = 4)


maaslin_bmi <- maaslin3(input_data = df_rel,
                           input_metadata = metadata_wreads,
                           output = './outputs/maaslin_results/bmi_reads',
                           formula = "~ BMI + Reads",
                           normalization = 'TSS',
                           transform = 'LOG',
                           cores = 4)

maaslin_bmi_cat <- maaslin3(input_data = df_rel,
                           input_metadata = metadata_wreads,
                           output = './outputs/ACVD_BMI_age_reads',
                           formula = "~ ACVD + BMI + Age + Reads",
                           normalization = 'TSS',
                           transform = 'LOG',
                           cores = 4)
```

```{r diffabund_genus, eval=FALSE, include=FALSE}
physeq_smg_genus <- phyloseq::tax_glom(physeq_rel, taxrank = "Genus")
physeq_smg_taxa <- as.data.frame(tax_table(physeq_rel)) %>%
  rownames_to_column(var = 'metaphlan_species') %>%
  select(metaphlan_species,Genus,Species)
abd_smg_genus <- as.data.frame(otu_table(physeq_smg_genus)) %>%
  rownames_to_column(var = 'metaphlan_species') %>%
  left_join(physeq_smg_taxa %>% select(-Species), by='metaphlan_species') %>%
  select(-metaphlan_species) %>%
  column_to_rownames(var='Genus')
df_rel_g <- t(abd_smg_genus)

# all samples, standard formula with random effect of participant
maaslin_genus1 <- maaslin3(input_data = df_rel_g,
                           input_metadata = metadata_wreads,
                           output = './outputs/maaslin_results/acvd_age_reads_genus',
                           formula = "~ ACVD + Age + Reads",
                           normalization = 'TSS',
                           transform = 'LOG',
                           cores = 4)
...
Differential abundance testing was performed using Maaslin3 with two different models:
$$ \sim ACVD + age + reads$$

$$ \sim BMI + reads$$

where reads is the total number of processed reads used for taxonomic abundance estimation. Differences in taxa prevalence (ie presence/absence) and relative abundance are tested separately and show up as two different rows in the results. The 'coef' column refers to either a 2^coef fold change in the relative abundance of the feature. In prevalence models, a one-unit change in the metadatum variable corresponds to a coef change in the log-odds of a feature being present.

### BMI

**There was not enough power to detect significant associations between BMI and gut microbiome taxa.**

### ACVD {#acvd}

Our initial analysis showed both higher prevalence and abundance of _Akkermansia muciniphila_ in individuals with ACVD. From this more robust analysis, the prevalence results (higher in ACVD cohort) remain significant, however the abundance of _A. muciniphila_ is actually not significantly higher, in fact it appears to trend lower (see negative coef value in table). There were no significant associations between _A. massiliensis_ and cardiovascular disease, however there was a similar trend of lower abundance of _A. massiliensis_ in the ACVD cohort, but the sample size was probably not large enough for significance.

This differs from our initial analysis of _A. muciniphila abundance_: if you compare the raw data the ACVD cohort has a higher MEAN relative abundance. The difference between the results is that Maaslin3 compares MEDIAN relative abundance.


<a id="akk-diff-table"></a>

```{r diffabund_tables}
maaslin_species_akk <- read.delim('./outputs/maaslin_results/acvd_age_reads/all_results.tsv') %>%
  filter(str_starts(feature, "Akkermansia")) %>%
  filter(feature != 'Akkermansia_biwaensis') %>%
  filter(!is.na(coef) & is.na(error) & metadata == 'ACVD') %>%
  mutate(sig = case_when(qval_individual < 0.05 ~ 'significant',
                         TRUE ~ 'not significant')) %>%
  select('Species'=feature,
         metadata,
         coef,
         'p.adj'=qval_individual,
         'significance'=sig,
         model,
         N_not_zero) %>%
  mutate(across(c(coef), round, 2)) %>%
  mutate(across(c(p.adj), round, 4)) %>%
  arrange(Species) %>%
  mutate(Species=replace(Species, Species == "Akkermansia_sp_N21116", "Akkermansia_guangxiensis"))
...
DT::datatable(
  ...
  caption = htmltools::tags$caption(
    style = 'caption-side: top; text-align: left; color:black; font-size:150%;',
    'Differentially abundant or prevalent Akkermansia by Maaslin3'),
  rownames=FALSE,
  width='100%',
  options = list(
    scrollX = '500px',
    ...
    info = FALSE,
    initComplete = JS(
      "function(settings, json) {$(this.api().table().header()).css({'font-size' : '14px'});}")
  )
) %>%
  formatStyle(columns = colnames(.$x$data), `font-size` = '14px')
```
...
We can look at other taxa that have different abundance between groups: this plot shows both abundance differences (log2 fold differences in relative abundance) and significance (log transformed q-values) to show that only a handful of taxa were differentially abundant. Points to the left of the vertical line are taxa enriched in the healthy cohort, and points to the right are enriched in the ACVD cohort.

<a id="acvd-table"></a>

```{r diffabund_tables2, fig.cap="Figure 3.5: Differentially abundant species"}
maaslin_species_all <- read.delim(
  './outputs/maaslin_results/acvd_age_reads/all_results.tsv') %>%
  filter(model == 'abundance' & !is.na(coef) & is.na(error) & metadata == 'ACVD') %>%
  mutate(sig = case_when(qval_individual < 0.05 ~ 'significant',
                         TRUE ~ 'not significant')) %>%
  rename('l2fc'=coef, 'p.adj'=qval_individual)

vol <- ggplot(maaslin_species_all,aes(x=l2fc,y=-log10(p.adj),group=feature)) +
  geom_vline(xintercept = 0,linetype='dashed',color='grey27') +
  geom_point(aes(color=sig,alpha=sig),size=3) +
  geom_point(data=maaslin_species_all %>% filter(str_starts(feature, "Akkermansia")),color='red',alpha=1,size=3) +
  scale_alpha_manual(values=c(1,0.25,0.25),breaks=c('significant','not significant','Akkermansia')) +
  scale_color_manual(values=c('grey','dodgerblue'),breaks=c('not significant','significant')) +
  my_gg_theme +
  theme(axis.text = element_text(size=12),
...
#maaslin_genus_all <- read.delim('~/Documents/WORK/2026.01.30_ACVD_metagenomics/maaslin_results/acvd_age_reads_genus/all_results.tsv') %>%
#  filter(model == 'abundance' & !is.na(coef) & is.na(error)) %>%
#  mutate(sig = case_when(qval_individual < 0.05 ~ 'significant',
#                         TRUE ~ 'not significant')) %>%
#  rename('l2fc'=coef, 'p.adj'=qval_individual)

#vol2 <- ggplot(maaslin_genus_all) +
#  geom_vline(xintercept = 0,linetype='dashed',color='grey27') +
#  geom_point(aes(x=l2fc,y=-log10(p.adj),color=sig,group=feature,alpha=sig)) +
#  scale_alpha_manual(values=c(1,0.25),breaks=c('significant','not significant')) +
# my_gg_theme +
# xlim(-5,5) +
# labs(x='Log2 Fold Change',
#      y='-log10(p.adj)')
...
This table shows all significant associations: species with a negative log2 fold change value have higher abundance in the healthy cohort when compared to the ACVD cohort.

```{r diffabund_tables3}
maaslin_table <- maaslin_species_all %>%
  filter(p.adj < 0.05) %>%
  select('Species'=feature,
       metadata,
       l2fc,
       p.adj,
       N_not_zero) %>%
  mutate(across(c(l2fc), round, 2)) %>%
  mutate(across(c(p.adj), round, 4)) %>%
  arrange(l2fc)

DT::datatable(maaslin_table,
              caption = htmltools::tags$caption(
                style = 'caption-side: top; text-align: left; color:black; font-size:150%;',
                'Differentially abundant species by Maaslin3'),
              rownames=FALSE,
              width='100%',
              options = list(
                scrollX = '500px',
...
Finally, we can also look for associations at the genus level: genera with a negative log2 fold change value have higher abundance in the healthy cohort when compared to the ACVD cohort.

```{r diffabund_table_genus}
maaslin_genus_all <- read.delim('./outputs/maaslin_results/acvd_age_reads_genus/all_results.tsv') %>%
  filter(model == 'abundance' & !is.na(coef) & is.na(error) & metadata == 'ACVD') %>%
  mutate(sig = case_when(qval_individual < 0.05 ~ 'significant',
                         TRUE ~ 'not significant')) %>%
  rename('l2fc'=coef, 'p.adj'=qval_individual)

maaslin_table_genus <- maaslin_genus_all %>%
  filter(p.adj < 0.05) %>%
  select('Genus'=feature,
       metadata,
       l2fc,
       p.adj,
       N_not_zero) %>%
...
DT::datatable(maaslin_table_genus,
              caption = htmltools::tags$caption(
                style = 'caption-side: top; text-align: left; color:black; font-size:150%;',
                'Differentially abundant genera by Maaslin3'),
              rownames=FALSE,
              width='100%',
              options = list(
                scrollX = '500px',
